...
[gear](gear.md) for the third progression axis, [history](history.md) for the roadmap and the
...
| **Tower**            | A second thing to climb: two hundred **floors** locked to one faction, whose enemies lean toward the faction that counters it. All seven ship, one per faction. A tower is a demand on the **roster** rather than on investment, is always skippable, and ⚠️ **never touches `clearedStages`, the ladder position or an idle rate.** See [authoring](authoring.md).                                                 |
