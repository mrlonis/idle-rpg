...
The full roadmap, shipped inventory, and design rationale per milestone live in
[`docs/milestones.md`](docs/milestones.md) — read it before starting work on a
milestone-scale change. The current milestone is **4. Team composition affecting combat
math**.

Two rules apply regardless of which milestone is current:

- **Do not skip ahead.** The milestones are ordered so there is always something playable,
  and a later milestone built on an unverified earlier one is where the hard-to-find bugs
  live.
- **Check where the project actually is before starting work.** Do not assume
  `docs/milestones.md` is current — verify against the code.
...
- Slow statistical balance sweeps belong in a separate vitest project (`*.balance.ts`) with
  ...
  yet because nothing has needed it: `data/stages.spec.ts` sweeps the whole ladder over 200
  seeds a stage — 1,600 battles — in about 10ms, which is what a headless `core/` buys. Create
