...
`data/stages.spec.ts` simulates two parties that differ in exactly one slot against stage 7 —
...
lost, it is a _category of answer_ the player can never buy. `characters.spec.ts` asserts both
