...
### 2. Auto-battle against a single stage — **NEXT**

`simulateBattle(team, stage, seed) => BattleResult` resolving instantly and headlessly into
an event log; the UI animates the log afterwards. Do not drive combat off the render tick —
that decision is what makes 2x/4x/skip and offline resolution free.

Use an ATB gauge (`gauge += spd` per tick, act at threshold) rather than fixed rounds, so
SPD is a real stat with real tradeoffs. Draw from a derived RNG stream
(`deriveSeed(seed, 'battle:<stageId>:<battleCount>')`) so replaying a battle is reproducible
and never shifts the pull sequence.

This milestone introduces `data/` (stage and enemy stat blocks).

### 3. Gacha: a few characters, seeded rolls, visible pity
...
Still outstanding, both blocked on combat existing:

- The **segmented** closed form for auto-progression advancing stages mid-window. Needs
  `timeToClear(state, stage)`.
- A drop source to feed `accrueDiscrete()`, plus the `dropCarry` field in `GameState` (add
  it via the first real migration, so the migration chain gets exercised for real).
...
- **Foreground/background handling via `@capacitor/app`.** The `visibilitychange` handling
  in `ui/game-loop.service.ts` covers the current need. Adding the native lifecycle plugin
  before combat exists means debugging two unfamiliar things at once.
...
- Expose pure functions. Shipped: `tick(state, dtMs) => state`,
  `resume(state, nowMs) => { state, report }`. Planned:
  `simulateBattle(team, stage, seed) => BattleResult` (milestone 2),
