...
- **[docs/combat.md](../docs/combat.md)** — the ATB loop, the damage formula, targeting, skills,
  ...
  - **Four status kinds arrived in milestone 17 — `taunt`, `reflect`, `link` and `bomb` — and they
    ...
    - **Milestone 21 licenses up to three new statuses across its four chapters, and 21a spent
      none.** The budget is a **ceiling rather than a quota** — a chapter that comes in under it is
      the better outcome — and the Waking Barrows found four unspent pairs instead: a taunt worn by
      a thorned body, a bomb aimed at `enemy-highest`, thorns on **fodder** (a row attack into three
      of them is answered three times, so the habitual reply to a crowded board is the expensive one
      there), and the first `ally-afflicted` turn on the enemy side. ⚠️ **Nothing may be forbidden
      to a later chapter on the strength of that** — 21b through 21d each argue their own spend in
      their own entry, and "21a managed without" is no more an argument than "17 did it" is.
    - **Milestone 21b spent the first of the three on `ROOTBOUND`, a partial permanent `link`, and
      the argument is what makes it allowed.** `spreadLink` matches partners on the **status id**
      rather than on the side, so a board can bind its back rank to itself and leave its wall out of
      it, and a lone holder takes the whole hit. That is what no pair of shipped parts says:
      `CHAINBOND` is cast, lapses and binds a whole side — a statement about focus fire in general
      — where this is aimed at one specific habit, that **reach** has been the answer to a protected
      back rank since milestone 4. Reaching a bound archer hands a third of the blow to the archers
      beside it, so the reach still works and buys spread rather than a kill. **Two remain.**
    - **Milestone 21c spent none of the two, and it found its four locks by aiming shipped parts
      somewhere they had never been aimed.** `THE_QUENCH` is the **first status of any kind on
      `enemy-lowest`** — which is where `ally-lowest` points every heal in the game, so a fuse lands
      on exactly the body the party's healer has just committed to; `IRON_FOR_IRON` is the first
      reflect applied to a _chosen_ ally and the first reactive one; `THE_ANVIL_FALLS` is the first
      stun aimed at **one body** rather than the whole board. The fourth is a pair — a taunt worn by
      a body whose `tenacity` means the one thing the party may hit is the one it cannot open. ⚠️
      **Two still remain, and 21d must argue its own**; "21c managed without" is no more an argument
      than "21a managed without" was.
    - **Milestone 21d spent the last two, so the budget is closed: `BLOODRISEN` and `SAVAGED`.**
      Both are permanent versions of something the game only had as a **window**, which is the same
      shape `ROOTBOUND`'s argument took and is now three for three. `BLOODRISEN` is
      `WRATH_UNBOUND`'s answer to being wounded with the forty-five tick timer taken off — a window
      is something a party weathers, and a permanent one turns the question into **how the party
      spends its damage**, since chipping five bodies arms five and finishing one at a time arms one.
      ⚠️ It is the deliberate inverse of `ROOTBOUND`: that punishes focus and this punishes
      spreading, so a party arriving from the weald holds the wrong habit. `SAVAGED` is the **first
      hostile status in the game that does not expire** — every debuff before it ran out on its own,
      so a cleanse was an optimisation and here it is the only exit.
    - ⚠️ **Both of 21d's are safe to be permanent for the reason `THORNMAIL` is: they can only ever
      shorten a fight.** One is extra damage on the party and the other extra damage from the board;
      neither puts health back. **The defensive mirror — a body that armoured itself as it was hurt —
      is the one shape of that idea nobody may author**, because it is the ninety-second clock with a
      narrative attached. Any future "the board answers being hurt" proposal has to clear that bar.
    - ⚠️ **The budget is spent and it does not renew.** Three of the four chapters came in under the
      ceiling, which is the outcome it was written for. A later chapter wanting a status argues from
      nothing exactly as 17 did: "21 had a budget" is not an argument, and neither is "21d spent
      two".
    - ⚠️ **A chapter's headline lock can be a stat block, and two chapters running have used one.**
      `ModifiableStat` is `atk`, `def` and `haste`, so neither `dodge` (the Sunless Weald) nor
      `tenacity` (the Hollow Anvil) can be a status without a `core/` change — and neither needed to
      be. What answers a dodge pool is `accuracy`, which five characters have carried since 8e; what
      answers a tenacity pool is `insight`, which **two** carry, and past that the answer the Adamant
      Colossus has always given: stop needing the debuff. `statusChance` is
      `authored + insight − tenacity` **clamped at zero**, so unlike `dodge` — which `MIN_HIT_CHANCE`
      floors at a tenth — a high enough pool refuses a debuff outright. **Reach for the stat block
      before the vocabulary**: the budget is for questions the existing parts genuinely cannot ask,
      not for questions that are inconvenient to author.
...
- **[docs/gear.md](../docs/gear.md)** — the third progression axis, added in milestone 12: five slots,
  ...
  - ⚠️ **`gradeSoftness` fires on every chapter forever, and the _shape_ rather than the number is
    what is wrong.** `gradeWeights` tilts by `1 + stageIndex / gradeSoftness`, so the top grade's
    share of end-of-ladder drops climbs without bound: it went past the `< 0.2` bound in
    `gear.spec.ts` at chapter 4 (90 → 100), at chapter 7 (100 → **125**, 24.5% → 18.7%), at chapter 8
    (125 → **150**, 23.4% → 18.7%), at chapter 9 (150 → **175**, 22.6% → 18.7%) and at chapter 10
    (175 → **200**, 22.1% → 18.7%). **Expect it once per chapter, forever** — chapter 11 wants 225.
    - ⚠️ **Five landings and every one of them is 18.7%, which is the finding rather than the
      numbers.** The solution is always `gradeSoftness = stages / 2` — the value at which the tilt
      equals exactly 3.0 — so the constant is not a tuning number at all, it is the ladder's length
      halved, written down by hand once a chapter. What it eventually wants is a tilt that
      **saturates**; that is recorded rather than taken because milestone 21 forbids the scope.
    - ⚠️ **Do not re-derive it several chapters ahead to save edits, and 21d is the evidence that
      call was right** — 21b declined 200, which would have bought the whole milestone, precisely
      because this quantity is _not_ meant to move and hiding it makes the real fix easier to forget.
      Had 200 been written then, the fourth and fifth landings would both have been silent, and it is
      the fifth that turned "always 18.7%" into "always `stages / 2`". That is the opposite of the
      call 21a made on `levels.spec.ts`'s ceiling ratio, which watched nothing for four chapters and
      was then **retired** in 21d rather than re-derived — so batching cost that one nothing. The
      distinction is whether the quantity is meant to fall. Raising the constant is safe for the
      starter wall and lowering it is not — see [gear](../docs/gear.md).
...
- **[docs/signature-items.md](../docs/signature-items.md)** — the fourth progression axis, added in
  ...
  - ⚠️ **No signature item may multiply healing, and this is the least intuitive rule in the
    ...
    is a `regen`.
  - **Shipped content can measure one since milestone 21b, and it took five milestones.** `mythic`
    caps at level **340** and the hardest authored stage is **588**, ×1.7 the unlock rung's cap, so
    the campaign is comfortably above it and moving further above it each chapter. `data/signature.balance.ts`'s
    `contested()` no longer re-levels the top encounter; it returns the stage as `data/` wrote it.
    ⚠️ **The Bound Marches, the Sundered Vault and the Waking Barrows were each expected to close
    this and none did** — 160, then 225, then 305 against 340, narrowing by less each time because
    the level band flattens as content lengthens. **Check the number rather than assuming the next
    chapter is enough** is the lesson that survives; the gap itself is closed.
  - ⚠️ **Measure a signature item by bisecting for reach, never by win rate at a chosen level.** The
    ...
    - ⚠️ **A reach figure is only comparable within one cut of the ladder, and this has now bitten
      four times.** `contested()` picks the hardest stage and seeds off its `stage.id`, so milestone
      19 renaming it `c4-s50` → `c6-s50` moved all seven recorded numbers with no item or stat block
      changed, and 21b, 21c and 21d replacing it with `c8-s50`, `c9-s50` and `c10-s50` moved all
      fourteen each time. **Re-measure the whole table or none of it, and expect to do it once a
      chapter for as long as chapters keep landing.** ⚠️ Removing the re-levelling in 21b changed
      _nothing_ by itself — `reach()` overwrites `level` on every trial, so the override was dead on
      arrival and what it bought was a claim about the method rather than an input to it.
    - ⚠️ **Read the _gain_ column, not the reach column — and do not predict which way either
      moves.** 21c recorded that bare reach rises fifteen to thirty levels between cuts while the
      gains stay still; 21d's cut saw bare reach rise **nought to seven** while five of the fourteen
      gains moved by five, Seraphine's among them after three cuts at +9. What survives is only the
      weaker claim: the gains move **less** (all fourteen within five, eight within three), and a
      reach figure means nothing outside its own cut. Re-measure rather than extrapolating.
...
- **[docs/economy.md](../docs/economy.md)** — the seven currencies, income rates, the level curve,
  ...
  income is `base × stageIndex ^ 1.45`, evaluated by `stagePayout` in `core/ladder.ts` from four
  coefficients in `data/chapters.ts`. **The exponent's job is that income tracks _enemy level_
  rather than stage count**, which is what stops a longer ladder from meaning a richer one — and
  that claim, rather than any value of the number, is what is calibrated.
  - ⚠️ **It was 1.13 until milestone 21b and the relation it protects had quietly inverted.** 1.13
    was set when enemy level was very nearly _linear_ in the stage index; 21a's corrected margin
    rule made it superlinear, so over the shipped ladder level grows as ~`index ^ 1.5` and income at
    `index ^ 1.13` was income proportional to `level ^ 0.80` — **sub**-linear, where the original
    calibration set it slightly super-linear at `level ^ 1.12`. At 1.45 it reads `level ^ 1.00`. The
    number moved to keep the statement true; the statement did not move.
  - ⚠️ **1.45 is the conservative end of a band.** Full restoration would be **1.60**; the guard that
    forced the change (`levels.spec.ts`'s `1h < hoursTo(top) < 24h`) needed only **1.42**. What it
    costs is legible — a stage-100 clear pays ×4.4 and a stage-10 clear ×2.1 — and it makes **no
    content easier**, because a party is capped by its ascension rung and not by its income.
...
- **The ladder is chapters, and where a run is, is a chapter and a stage within it.** Read
  ...
  - **Ten chapters ship — 10, 20, 30, 40 and then six of fifty, four hundred stages in all.** The
    first six are the two hundred the four-chapter cut carried, re-cut in milestone 19 so the
    boundaries land where a session does; the last four are milestones 21a through 21d. The curve is
    ...
    the Fenlord, the Pale Warden, the First Cinder, the Ashfall Sovereign, the Chainsworn, the Hollow
    Seraph, The Cairn King, The Withered Crown, The Anvil Crowned and The Everwound — a re-cut that
    moves a boundary owes the new final a unique body before it ships. ⚠️ **The rule is about the
    _headline_ body and nothing else** — a lieutenant may stand on its chapter's final as support and
    may not _be_ it. The Gravewright does on `c7-s50`; the Longshadow, the Grudgekeeper and the
    Redmaw do not, and all four are correct: a comment claiming the first was "deliberately absent"
    survived a whole milestone, and removing it measures as a chapter final 6% harder than the stage
    before it that the tuned party clears with all five alive. Auto-battle unlocks when chapter 1
    falls (`AUTO_BATTLE_UNLOCK_CHAPTERS = 1`, resolved through `chaptersCleared`), and all seven
    towers open with it.
  - ⚠️ **Two guards that measured "the ladder must not consume the curve" were retired in 21d, and
    one claim in rungs replaced both.** `chapters.spec.ts` held the top stage under
    `LEVEL_CURVE.maxLevel / 2` = 500 and `levels.spec.ts` held a ratio of what the ceiling costs to
    what the top stage demands, floor 4; chapter 10 closes at **588** and reads **3.62**. Neither was
    satisfiable by any chapter the margin rule permits — `ascended` caps at 500 and a chapter closing
    below its own rung's cap is the walkover 21a measured — and **both quantities fall on every
    chapter by construction**, so moving either buys one chapter at the price of pretending it still
    watches something. What owns the claim now is `levels.spec.ts`'s **"leaves rungs unspent above
    everything the ladder asks for"** (top < `caps[12]` = 700), which cannot decay because the rung
    count is fixed. That is the third guard retired rather than slid, after the absolute
    hours-to-the-ceiling and the ratio that replaced it.
    - ⚠️ **What 21d measured and deliberately did not guard**: the level line adds ~90 levels a
      chapter (80, 91, 94, 98), so the rung claim fires at **chapter 12** and the curve is consumed
      around **chapter 15** — not the chapter 100 every one of these guards was written on, a premise
      untrue since 21a's correction. **How long the campaign is meant to be is a roadmap decision**,
      and it is the question to answer when that guard fires rather than a number to move.
  ...
  - ⚠️ **A chapter that asks for a new ascension rung has to out-climb the rung it asks for**, and
    ...
    Twenty-three levels is what ×1.6 costs at `perLevel.common`. Chapters 1 through 4 never met this
    because each ran inside a cap the party already had.
  - ⚠️ **The margin has to _grow_ by about twenty-three levels a chapter, and a constant one cancels.
    Milestone 21a found this by authoring a chapter to the wrong rule and measuring it.** Each
    chapter hands the party a fresh rung (×1.6) **on top of** the levels it climbs, while the content
    climbs only the levels — so a fixed +25 deficit is paid once and never again, and the gap
    compounds. Party power ÷ the difficulty probe's threshold at each chapter's final: **1.08** at
    chapter 5 (+20), **1.44** at 6 (+25), and **2.08** at a chapter 7 authored at +25. At that
    setting every stage in the chapter was a walkover for the party it was tuned for — the reference
    five took the boss with all of them alive in seven seconds — and ⚠️ **no board fixes it**: heavy
    enough to cost that party a member meant **three** ascended bodies on one board, at 1.86× the
    stage before it. The margins are **+20, +25, +45, +56, +70, +88** — the Waking Barrows close at
    **305** against `legendary-plus`'s cap of 260, the Sunless Weald at **396** against `mythic`'s
    340, the Hollow Anvil at **490** against `mythic-plus`'s 420, and The Bleeding Wild at **588**
    against `ascended`'s 500.
    - ⚠️ **"About twenty-three a chapter" is right in shape and too large in size, and milestone 21b
      found that the same way 21a found the rule it replaced — by authoring a chapter to it and
      measuring.** At the roadmap's 411 (a margin of +71) the party the chapter is tuned for took the
      final at **0%**. The correction is the **enemy's own growth curve**: 21a derived +23 from
      `perLevel.common` because that is what the party climbs on, but an enemy `ascended` block
      climbs on `perLevel.ascended` — 1.024 against 1.021 — and that gap compounds over the _whole_
      level rather than over the chapter. `(1.024 / 1.021) ** 411` is ×3.34 where `** 305` is ×2.45,
      so the enemy side takes about **fifteen levels' worth** of head start across one chapter at
      this depth, straight off the margin: +23 − 15 ≈ **+8**, and +45 → +56 is +11. The rule is
      "+23 a chapter _less whatever the enemy's own curve has taken_", and the second term grows —
      so the roadmap's ~514 and ~617 for chapters 9 and 10 are both too high.
    - ⚠️ **And the corrected rule under-predicts, which milestone 21c found by measuring it.** Solving
      21b's own arithmetic forward reproduces chapter 8's shipped 396 to within a level and points
      chapter 9 at 486 — where the tuned party takes the final with **4.75 of five still standing**,
      which is a walkover by the standard 21a set. Measured, the close is **490** and the margin is
      +70 rather than the predicted +66. **The arithmetic is a starting bracket and nothing more**;
      it has now been wrong in both directions, low here and high in 21b.
    - ⚠️ **Four measured margins put the growth at +12 to +18 a chapter and rising, not +8**, so
      21b's correction is a real term rather than the whole answer. 21d's chapter was estimated at
      ~570 and closed at **588**; at 570 the tuned party took the final with all five alive in eight
      seconds. **Every closed form tried across this milestone has been wrong. Bisect; do not
      solve.**
    - ⚠️ **Check it by measurement, and the arithmetic is only for knowing which way to guess.** The
      transition is a **step function** — chapter 8's final read 100% at level 398, 85% at 400, 45%
      at 402 and 0% at 408, and chapter 9's read 100% at 496, 85% at 498, 25% at 500 and 0% at 502 —
      so bisect for the 90% edge, back off to where the tuned party keeps three or four of five, and
      then check the probe threshold at the final against
      `pow(1.021, level - 1) * pow(1.6, rungsAboveRare)`, which wants to land near **1.1–1.4**. The
      shipped ratios are 1.16, 1.10, 1.21 and **1.15**; chapter 10's final read 100% at 592, 85% at
      594, 43% at 596 and 13% at 598.
    - ⚠️ **The difficulty probe's bracket has to be widened with the level line, and the step count
      with the bracket.** The probe brackets the _party's_ power, which is
      `1.021 ** (level - 1) × 1.6 ** rungs` — so it grows with the margin rule rather than with the
      stage count. 21b took it 4,000 → 50,000, 21c → 500,000 with two more steps, and 21d →
      **5,000,000** with a seventeenth; expect the ceiling to move every chapter at this depth. ⚠️
      **The two halves move on different schedules**: a factor of ten on the range costs almost
      nothing in resolution (0.030% → 0.028%) while one extra step nearly halves it (→ 0.014%), so
      widen every chapter and add a step every few. Not widening at all presents as a difficulty
      curve flattening into a horizontal line at the ceiling.
    - ⚠️ **The board is not the variable and cannot be the fix.** Chapter 7's own final re-levelled
      to 411 also reads 0% for the party chapter 8 is tuned for, and chapter 8's final at 380 reads
      100%. When a chapter measures unclearable at the top, the level line is what moved.
  - **A rung per fifty-stage band is the cadence** — it read "one rung per chapter" until milestone
    ...
    ladder: 20 copies by the end of the fen's fifty stages, 24 by the Marches, 32 by the Vault, 37
    by the Barrows, 46 by the Weald, 52 by the Anvil, 62 by the Wild. ⚠️ **Three of those were wrong
    by one or two until 21d measured them** (38/44/50), which is the ordinary cost of quoting a
    running total by hand — `MORTAL_LADDER` alternates cheap and expensive rungs (3, 7, 4, 8, 5, 9,
    6, 10), so the step between chapters alternates 6 and 10 rather than being constant, and
    "six more than the last chapter" was true twice and then repeated on faith. ⚠️ **It is the assumption under the
    levelling-versus-ascension guard** — a band
    climbs ~65 levels and a rung only pays for 22.6, so the two axes drift apart by construction and
    the guard measures the **share** rather than the ratio for that reason.
  - **Adding a chapter is an economy change as much as a content one**, and expect four or five
...
- **Faction towers are a second thing to climb**, and **all seven ship** — the system in milestone
  ...
  - ⚠️ **All seven are two hundred floors as of 21k, and how the interim was handled is the rule to
    reuse.** `TOWER_RULES` is one rule for all seven, so the height moved in one session (21e) while
    the floors moved in seven (21e–21k). A tower not yet extended was **not damaged** —
    `clearedFloors` clamps to what it authors, so `nextFloor` reported it topped and every screen read
    it right — but it lost its **boss**, because `floorKindAt` reads the rules' height and its
    hundredth floor resolved as a mini-boss paying ×2 rather than ×5. What kept that honest was
    `PENDING`, a **literal list of names that deletes itself**, in both `towers.spec.ts` and
    `towers.balance.ts`; each session removed its own and 21k removed both lists along with the
    branches they guarded. ⚠️ **A filter — "either the full height or half of it" — would have passed
    forever and never noticed a tower nobody went back for. Do it the same way if the height moves
    again.**
  ...
  - **Every faction carries at least six archetypes and all three tiers**, which is what 15c's
    ...
    A hundred and thirty ship, and the distribution is deliberately uneven: milestone 18 took Angels to
    ...
    chapters' leans up front** — undead, elf, dwarf, monster, the four thinnest — so the four sessions
    ...
    hundred took it to **thirteen**, so no faction is now a standout candidate to lean on; 21g took
    Dwarves 18 → **twenty-two** and 21h took Elves 17 → **twenty-one**. ⚠️ **A
    tower session deepens a faction exactly as a chapter session does, four blocks against ten** —
    that is the other half of the "fix the leans up front" discipline and it held through 21k;
    21i took the four thinnest a block each, 21j took Demons 13 → **seventeen**, and 21k took Angels
    12 → **sixteen**. ⚠️ **A count quoted in the roadmap ahead of time goes stale the moment a session
    spends a block outside its own lean** — 21i's spread block took Demons to 13, so 21j's four landed
    on 17 against a roadmap that said 16, and 21k landed on 16 against a roadmap that said 15. The
    rule did not move; the count it was quoted against did. **Recompute the depth table rather than
    reading it.** The seven now run **human 14, angel 16, demon 17, monster 19, undead 21, elf 21,
    dwarf 22**, and Humans are the thinnest.
    - ⚠️ **A tower with no lean spreads its four blocks the way it spreads its boards, and 21i is the
      case.** The Monster Tower took one block each in the four thinnest factions — angel 11 → 12,
      demon 12 → 13, human 13 → 14, monster 18 → 19 — rather than four into one. Deepening the
      thinnest is not decoration there: 21j took Demons to 17 and 21k Angels to 16, and **neither
      touched Humans**, so without that session Human would have closed milestone 21 as the standout
      thin faction at 13 against Dwarf's 22 — the state 21f exists to have got the game out of.
      **Check what the remaining sessions already cover before choosing which faction to deepen.**
      The old
      ...
      bounded by the ones the campaign already fields** rather than by an opinion — the Unmade is the
      ceiling and nothing may reach it, asserted in `enemies.spec.ts`, because a third and fourth heavy
      anchor is what makes six towers fail their sweep at once. **Every chapter boss since respects that
      ceiling rather than raising it**: the Chainsworn, the Hollow Seraph, The Cairn King, The Withered
      Crown, The Anvil Crowned and The Everwound are each authored under the Unmade on both stats, and
      what makes them the harder fights is the questions they ask and the level they are fielded at.
  - ⚠️ **A faction's own idiom can be what makes it hardest to author, and Dwarves are the case.**
    They own the tankiest blocks in the game, so a chapter leaning on them is the one most able to
    run the ninety-second clock out — against a timer whose headroom over the longest tuned fight is
    1.44×. The Hollow Anvil's ten blocks are sized against that rather than against the fiction:
    commons at or below the weald's bulk, single-digit resists where another faction would carry a
    fifth, three of the ten carrying real `haste`, and no board fielding two of the Colossus, the
    Cairn Sentinel and the Iron Bulwark at once. **What makes a Dwarven chapter hard is refusal, not
    health.**
    - **Monsters are the opposite case and needed the opposite discipline.** The faction's idiom is
      raw `atk`, thin armour and `lifeLeech` in place of a healer (milestone 8e), so The Bleeding
      Wild's boards resolve fast — its longest fight is 15.5 seconds — and the risk moves from the
      clock to a **closed loop**. ⚠️ **The two rules that fence it**: leech never stands on a board
      with a taunt, because sustain the party cannot aim at is the timeout again; and leech never
  ...
  - **A chapter also owes a _lieutenant_ since milestone 21**: one heavy block anchoring all four of
    ...
    twenty blocks each appearing on exactly one board is most of what the orphan rule in
    `enemies.spec.ts` exists to discourage. The Gravewright is the first, then the Longshadow, the
    Grudgekeeper and the Redmaw. ⚠️ **A lieutenant's signature does not have to be an opening turn,
    and the last two are reactive.** The first two open with a board-wide status at priority 5, which
    sets their board up on tick one and then stops; the Grudgekeeper cannot, because its chapter's
    first band is a stat block, and the Redmaw cannot, because a chapter about what the party's
    damage _does_ has nothing to say before the party has dealt any. Both carry a **conditioned**
    skill instead, and that is the better shape — it answers what the party is doing, so four
    appearances are four different fights against one block.
  ...
  - ⚠️ **In band 2 the two arrangements come apart, and _which_ of them binds is a per-tower and
    even a per-board answer.** With both crews at parity with the content this never arose. On the
    Human and Elf Towers the **alternate** five is the whole constraint — the pairs measure twelve
    and **nine** levels apart, the Elf roof board reads 100% for the alternate at level 118, 83% at
    120 and 2% at 126 while the reference five is still at 100% at 126, and every board that costs
    the reference crew a second member takes the alternate below its own 75% bar. ⚠️ **21h found the
    Undead pair swapping places by mechanic**: in band 1 the alternate is far the stronger (floor 100
    costs it 1.6 of five against the reference crew's 3.0), and on a `dodge` board at the roof's
    level it is far the weaker (65% against 95%), because that arrangement's kit is three
    single-target drains. **Check both arrangements on every candidate board** rather than picking
    one to size against; "size it against the alternate first" is true of two towers and not a rule.
  - ⚠️ **How a second hundred escalates is a per-tower answer and seven towers gave seven
    ...
    health rather than time. ⚠️ **The Undead Tower's answer is the first that is structural rather
    than a matter of weight**: at the roof's level, controlled at one anchor plus two legendaries and
    two commons, `dodge` reads 95%/**65%** where burst reads 100%/95%, a healer 98%/90%, and slow,
    link and reach all 100%/100% — the only shape that fails a bar. **No Undead character carries a
    point of `accuracy`** (it is on four Elves and one Human, and in no gear archetype or signature
    item), and every Undead body sustains on `drain` and `lifeLeech`, so a miss costs the hit _and_
    the health it would have returned. Measure the candidate shapes against both of that tower's own
    crews before authoring a floor.
    - ⚠️ **The Monster Tower's answer is the first that is a _count_ rather than a shape**, and it is
      the fifth in five sessions. Controlled at one anchor, one legendary and three commons at the
      roof's level, mean survivors of five: nothing 4.35/4.00, one lock repeated four times
      4.13/3.92, three questions 4.00/2.70, **five questions 3.58/0.85**. Repeating a lock is worth
      almost nothing and the count is worth everything — a Monster five answers any single question
      by out-damaging it and has no second answer to spend on two more — so the bands escalate two →
      three → four → five distinct questions. **Monsters are the only faction in the game with no
      heal, no regeneration and no shield**; sustain is `drain` and `lifeLeech` alone, and no
      character carries `tenacity`, `accuracy` or `dodge`.
    - ⚠️ **A link is a defence against focus fire, so it makes a board _easier_ against a crew that
      spreads.** On a five-question board `rootbound` took the weaker Monster arrangement from 2.42
      ...
      that tower carries one. **Check what a crew's kit actually does before reaching for a link.**
    - ⚠️ **A lock being unanswerable is not the same as it being this tower's.** No Monster character
      carries `accuracy` either, so `dodge` reads 100%/50% there — the same structural gap 21h built a
      whole second hundred on. It was left on the shelf: **two towers with one lock is one tower
      shipped twice.**
    - ⚠️ **The Angel Tower's answer is the sixth, and it is the first where the honest finding was
      that _no mechanic is available at all_.** Twenty-two shapes were measured against both Angel
      arrangements at the roof's level, controlled at one anchor plus four bodies all asking the same
      question — taunt, thorns, link, bomb, `SAVAGED`, `BLOODRISEN`, `dodge` 0.30, `tenacity` 0.60, a
      board stun, a board slow, a shield, `magicResist` 0.40, a healer, hex volume — and the **whole
      spread was 0.15 survivors of five**, every row between 3.92 and 4.00. An Angel five is
      `GUARD`, `BARRIER`, `AEGIS`, two or three heals and a cleanse, and the first question is free.
      ⚠️ **21i's count of distinct questions does not transfer for exactly that reason**: it works
      because a Monster five answers any _one_ question by out-damaging it and has no second answer,
      and here the first one costs nothing either.
      - **What moves them is tempo and aim.** Naming `enemy-lowest` takes the alternate 4.00 →
        **2.00**; `haste` 140 on a thin body takes it to **0.15**; reaching `enemy-back` 3.10;
        naming `enemy-highest` reads **4.50 / 4.33**, i.e. _easier_, because that is where the two
        tanks stand. Every Angel heal names `ally-lowest` and every Angel shield is behind a
        cooldown or an energy bar, so a board that arrives before the ward and spends itself on the
        body the choir has just committed to is racing the crew's own cooldowns. ⚠️ **`haste` on a
        _durable_ body is worth almost nothing** (4.00 → 3.75 at 160) — it is `haste` on a **thin**
        one that is the strongest dial there is, so softness is the mechanic's price rather than a
        discount on it.
      - ⚠️ **Both dials at once is past the edge**: fast _and_ naming the lowest reads 0.00 / 0.00.
        Aim and speed arrive one band apart for that reason, and the closing band carries no more
        than two bodies above `haste` 126.
      - ⚠️ **This is the first tower whose two arrangements fail on _opposite axes_**, which is a
        different thing from 21h's pair swapping by mechanic. **Weight breaks the reference five** —
        any two of the Unmade, the Ashfall Sovereign and the Hollow Seraph on one board reads 5% or
        below for it against 15–65% for the alternate — and **length breaks the alternate**, whose
        five characters field _four_ damage skills between them at `elite` and which is the slowest
        party in the game. So neither crew is "the" constraint and the roof has to thread both bars;
        decide that before authoring rather than discovering it in the sweep.
      - ⚠️ **Denial is a cost on that tower rather than an escalation.** A healer, slow, shield or
        resist wall leaves both crews at 4.00 survivors and buys only seconds — four healers take the
        alternate 26.0s → 37.8s against a 67.5s cleared-fight bar. Its bands therefore forbid a heal,
        a regeneration, a drain **and** `lifeLeech` above floor 160, which is what pushed the Headsman
        (`lifeLeech` 0.2) out of the last two bands. **A faction that cannot burst turns every second
        of enemy sustain into the clock**, which is 21f's rule biting harder than on the crew it was
        written for.
    - ⚠️ **The Demon Tower's answer is the seventh and the last, and it is about _scope_ rather than
      about a mechanic.** Controlled at one anchor plus four bodies all asking the same question at
      the roof's level, forty seeds, against a 4.13 / 4.05 control: stun, slow, weaken, sunder,
      poison, `SAVAGED` and `HEXBRAND` aimed at **one body** read 4.17–4.38 / 4.05–4.17 — seven
      mechanics, every one of them leaving the board _easier_ than saying nothing — and a taunt reads
      **4.78 / 4.85**, easier still. The same statuses riding a **board-wide** turn read 4.03 / 2.88
      (slow) and **3.95 / 1.85** (stun) against 4.53 / 3.88 for the wide damage alone. The reference
      five carries 9,416 to 12,822 hp a body at `elite`, so a question put to one of them is a turn
      the other four do not have to answer. Its bands escalate one voice → a voice with a rider → the
      rider becoming the turn → two voices → three.
      - ⚠️ **The status must ride the attack rather than cost a turn** — the same statuses on a skill
        of their own are the first table. And ⚠️ **this is a fact about these two crews, not a
        structural gap only Demons have**: the identical board reads 2.40 / 0.60 against the Elf
        crews and 0.88 / 0.00 against the Monster crews. The control that makes it this tower's is the
        Angel five, where it reads 4.00 / 3.95.
      - ⚠️ **Weight is not available there at all.** The Unison beside a Hierophant reads 95% / 3.17
        for the reference five and **5%** for the alternate; beside a Colossus 70% / 0%. No board in
        that hundred carries two `ascended` blocks.
      - ⚠️ **The magic ward was the obvious axis and it was declined on size, which is worth
        remembering because someone will find it again.** Demons are the only faction with **zero
        physical damage skills** (19 magical / 0 physical; Undead are next at 14 / 6) and no stat
        counters `magicResist` — at 0.60 a wall reads 3.21 / 1.46 there while leaving five of the
        seven crews within 0.25. But the highest `magicResist` on any shipped block was **0.14**, and
        at 0.15 the same wall is worth 0.00 / 0.54. **Check a stat's shipped register before building
        a band on it.**
    - ⚠️ **A mistyped optional stat is silent in both directions, and 21j found three shipped by
      21i.** `magicalPierce` on two blocks and `magicalResist` on one — the fields are `magicPierce`
      and `magicResist` — so all three did nothing and no test noticed: `data/` asserts conformance
      by assigning `ENEMIES` to a typed local, and an object already frozen with `as const` is not a
      fresh literal, so TypeScript's excess-property check never runs on it. They were **deleted**
      rather than corrected, because deleting is the zero-behaviour-change fix and keeps every figure
      the earlier session recorded valid. **Audit the keys with a script over `StatBlockData`
      whenever a session authors stat blocks**; the typed local does not cover this.
    - ⚠️ **A taunt on a soft body is a gift, not a door — the mechanic's price is its carrier's
      health.** 21i authored a common taunt on the argument that a taunt narrows the pool before the
      row rule is consulted, and measured it taking the reference five from 4.42 survivors to
      **4.70**. A body the party kills in a turn is a **cheap target volunteered**, and a
      multi-target selection ignores a taunt entirely. Every taunt in the game is a legendary
      carrying 1020 to 1180 hp, and that is why.
    - ⚠️ **`def` is not a lever at these levels, and a shield does not deny a drain.** 21i measured an
      "armoured runt" — a small HP pool behind a huge `def`, to magnetise the three `enemy-lowest`
      drains onto a body returning nothing — at hp 300 / def 70 and it read **identical to a plain
      common**; two of them made the board easier. Damage is `atk² / (atk + def)`, so at a roof's
      level the crew's `atk` swamps anything authorable. And `simulate.ts` takes leech off damage
      **dealt, shield included** — only thorns is measured against what reached HP.
  - ⚠️ **A lock the crew cannot buy an answer to is licensed by _where it is put_, not by its size.**
    Evasion pools go on soft bodies — the Sunmote Dancer is 500 hp at `dodge: 0.3` — so reach and
    focus fire are the answer, which is the case 21g made for the Plumbline Hand's `accuracy` in the
    mirror direction. The heaviest body on that tower carries **less** evasion than the legendaries
    around it. And `dodge` is a chance floored by `MIN_HIT_CHANCE`, unlike `tenacity`, which refuses a
    debuff outright: it costs turns and never closes a door.
  - ⚠️ **The bottom of a band 2 is not a fight, but how little composition buys there is a fact
    about the _crew_ rather than about the band split.** Band 2's crew stands at level 100 while the
    band opens at **61** — a 39-level deficit worth ×2.24 of party power — so the level line has not
    caught up and the escalation belongs in the last thirty floors either way. But the size of the
    effect does not transfer: at the Elf Tower's floor 101 the lightest authorable board resolves in
    2.6 seconds and the heaviest in **2.9**, three tenths of a second across the whole range, and
    against the Undead pair the same measurement reads **2.7s and 8.4s** — a threefold span. 21g
    concluded "composition buys nothing down here" from an Elf five, which is the fastest party in the
    game; **measure it on the tower's own crew before deciding how much the opening bands may carry.**
    Author them for rhythm regardless.
...
- **Achievements and quests are ledgers over counters the run already keeps**, added in milestone
  ...
  - **Nothing in the crystal economy is linear in the stage index.** A first clear pays a flat 250
    ...
    - ⚠️ **Only the ceiling on that ratio is stable; the floor falls as content ships, by
      ...
      **0.74** — the last two landing on the figures 21b projected for them, to two decimal places.
      ...
      21f's Dwarf Tower took it to **0.940**, 21g's Elf Tower to **1.045** — the first reading back
      over parity with the campaign since chapter 8 — 21h's Undead Tower to **1.150** and 21i's
      Monster Tower to **1.255**, 21j's Angel Tower to **1.361** and 21k's Demon Tower to **1.466**.
      ⚠️ **The step is exactly 31,300 crystals — one tower's second hundred — so it is exactly
      +0.1052 every time by construction**, which is why it read the same seven times running. Do not
      verify it by subtracting the rounded ratios; 1.255 → 1.361 looks like +0.106.
    - **The floor is back at 1.3, which is where it stood before milestone 21 rather than a new bar.**
      Milestone 21 _was_ the "grow the towers" answer and it landed in eleven sessions: 21a–21d took
      the campaign to 297,500 while the tower side stayed at the shipped 219,100, and 21e–21k doubled
      every tower to **436,100** — the first time since this guard was written that both sides moved.
      ⚠️ **The floor was lowered once, to 0.7, covering 21b–21d in a single edit** — the three
      landings (0.96, 0.83, 0.74) were all known in advance, so re-deriving a quantity that is
      _supposed_ to fall three times over would be three edits that measure nothing. That is the same
      call 21a made on the level ceiling's ratio and the **opposite** of the one 21b made on
      `gradeSoftness`; the distinction is whether the quantity is meant to move. The acknowledged
      price was that it watched nothing from 21b until 21k restored it.
    - ⚠️ **1.3 rather than 1.4, and the reason is the next chapter rather than the measured 1.466.**
      This ratio falls again as soon as the campaign grows: an eleventh fifty-stage chapter takes it
      to **1.314** and a twelfth to **1.190**, so 1.3 survives chapter 11 and fires at chapter 12
      where 1.4 would have fired on the very next chapter shipped. **A failure there is the original
      question rather than a number to slide** — the towers are no longer fixed while the campaign
      grows, so the honest answers are a third hundred, an eighth ladder, or accepting that the
      campaign has outgrown its optional content.
  ...
  - ⚠️ **`perClearPerHour` is 1 and it stays 1. The ceiling that kept catching it was the thing that
    ...
    - **The ceiling is stated against the roster now, not the ladder**: a full clear must not buy the
      ...
      thirty days. It is **~41 days** over the shipped four hundred stages — 47 at chapter 9, 52 at 8,
      60 at 7 and 70 before that, and milestone 20 was the first time the **roster** side moved it
      (+551 copies, 62 → 70 days). ⚠️ **It tracks both sides**, so a roster that grows
  ...
  - ⚠️ **What a clear pays is bounded _per stage_, not in total.** The band was 500–900 pulls for
    ...
    chapter boundaries without touching a stage, which milestone 19 records as a decision; each
    fifty-stage chapter since dilutes it slightly (7.76 at chapter 7, 7.62 at 8, 7.51 at 9,
    **7.44** at 10).
  - ⚠️ **The level ceiling is guarded by _rungs_ now, and both hour-based guards have been retired.**
    "Level 1000 costs more than 500 hours of top-of-ladder income" went first: income at the top rises
    with every chapter **by design**, so that figure falls forever (1,175 → 588 → 372) and would reach
    a weekend around chapter twelve with nothing wrong. Three assertions replaced it — the ceiling
    against what the ladder asks for, rungs left unspent above that demand, and the demand itself
    costing between an hour and a day. **Before retuning content to satisfy a failing threshold, check
    whether the quantity it measures is one the roadmap requires to move.**
    - **The first of the three was _meant_ to fall, milestone 21a cut it four chapters' worth at once,
      and 21d retired it**: 37.1 at chapter 6, 17.8 at 7, 9.4 at 8, 5.6 at 9 and **3.62** at 10,
      against a floor that had already moved 25 → 4. ⚠️ **A ratio one derivative out from a quantity
      that must fall is still a quantity that must fall** — that is the whole lesson, and it took two
      retirements to learn. Its own comment prescribed asking, when it fired, "whether the ladder has
      come far enough to have earned the distance it has closed"; that is not a question a threshold
      can answer, so what remains is the structural claim alone. **Income cancels out of this ratio**,
      so raising the reward exponent in 21b did not move it, which is why moving the exponent could
      not have saved it either.
    - ⚠️ **The third of the three fired at chapter 8 and was answered by moving the income
      exponent, not the threshold.** `1h < hoursTo(top stage) < 24h` read 10.2 at chapter 6, 21.7 at
      7 and **41.4** at 8. Level cost grows as `L^2.55` while top-of-ladder income grew as
      `index^1.13`, so the quantity was decaying by construction — but the guard carries a real
      design claim ("a day of income is the wall rather than the content") and 21b treated an
      overshoot of that size as evidence about the cadence rather than as a number to slide.
      - **Three alternatives were measured and all three fail.** Flattening the essence curve is
        arithmetically insufficient (below an essence exponent of 2.0 the binding currency becomes
        **xp**, which alone reads 27.8h at chapter 9); scaling `baseRates` buys about one chapter per
        doubling, because the divergence is between two exponents; and ⚠️ **there is no level at
        which a chapter satisfies both the margin rule and the guard** — 24 hours landed at level
        ~330 against `mythic`'s cap of 340, so chapter 8 would have had to close _below_ the cap of
        the rung it asks for, which is the walkover 21a measured.
      - The exponent went **1.13 → 1.45** and it reads 6.7 at chapter 8, 11.2 at 9 and **16.1** at 10. See the economy section above for why that is a derivation rather than a fudge. ⚠️ **This
        is the only one of the three still watching anything**, it climbs about +4.7 a chapter, and a
        third of 1.45's headroom is left — so **chapter 11 is where income is the question again**,
        and this time there is no retired guard to fall back on.
